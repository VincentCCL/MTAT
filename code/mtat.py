...
def repair_json_like_text(text: str) -> str:
    # Repair the common LLM error: ["a"; "b"] instead of ["a", "b"]
    text = re.sub(r'"\s*;\s*"', '", "', text)
    
    # Also repair semicolon before a closing object/list if ever produced
...
def parse_openai_translation_output(text: str, expected_n: int) -> List[str]:
    ...
    for candidate in candidates:
        candidate = repair_json_like_text(candidate)
        try:
...
def translate_openai_batch(
    ...
    print(
        f"[error] batch failed after {max_retries} attempts: {last_error}",
        file=sys.stderr,
        flush=True,
    )

    # Fall back to sentence-by-sentence translation
    if len(batch) > 1:
        print(
            f"[warn] retrying batch of {len(batch)} sentences individually",
            file=sys.stderr,
            flush=True,
        )
        ...
        for sentence in batch:
            try:
                ...
                print(
                    f"[error] sentence failed completely: {e}",
                    file=sys.stderr,
                    flush=True,
                )
                outputs.append("")
    ...
    # Single sentence also failed -> return empty translation
    return [""]
